...
export class BotApp {
	...
	init() {
		...
		this.bot.on('message', async (ctx: Context) =>
			this.allMessagesHandler(ctx, this.options.aiClient, this.options.notionService, this.telegram, this.options.azureTableClient.messages, 'natural'),
		);
		this.bot.catch((err) => {
	...
	private async handleMessageText(
		...
		// }
		// if (countNoResponse === response.length) {
		//   await ctx.reply(t.sorryICannotUnderstand);
		//   return;
		// }
	}
